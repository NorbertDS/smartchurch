...
router.get('/landing', async (_req, res) => {
  try {
    const s = await prisma.setting.findFirst({ where: { key: 'landing_config' } });
    if (!s) {
      return res.json({
...
router.get('/stats', async (_req, res) => {
  try {
    ...
    const [members, events, departments, entries] = await Promise.all([
      ...
      prisma.attendanceEntry.findMany({ where: { attendanceRecord: { date: { gte: since } } } }),
    ]);
...
router.get('/church-info', tenantContext, requireTenant, async (req, res) => {
  try {
    const tid = (req as any).tenantId as number;
    const s = await prisma.setting.findFirst({ where: { key: 'church_info', tenantId: tid } });
    if (s) {
      try {
        const info = JSON.parse(s.value);
        const name = String(info?.name || '');
        const abbr = String(info?.sidebarName || (name ? name.split(' ').map((w: string) => w[0]).join('').toUpperCase() : ''));
        return res.json({ name, abbreviation: abbr });
      } catch {}
    }
    const t = await prisma.tenant.findUnique({ where: { id: tid } });
    const name = t?.name || '';
    const abbr = (t?.slug || '') || (name ? name.split(' ').map((w: string) => w[0]).join('').toUpperCase() : '');
    return res.json({ name, abbreviation: abbr });
  } catch (e: any) {
