...
interface CommitteeMember { id: number; role?: string; member?: Member }
interface Committee { id: number; name: string; responsibility?: string; chair?: Member | null; members?: CommitteeMember[] }
...
export default function CommitteeDetails() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await api.get(`/committees/${id}/resources`);
      setResources(Array.isArray(r.data) ? r.data : []);
    } catch (e: any) {
      setError(e?.response?.data?.message || 'Failed to load committee');
  ...
  const upload = async () => {
    ...
    try {
      ...
      await api.post(`/committees/${id}/resources`, fd);
      setFile(null);
      await load();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-4">
      {loading ? (<div>Loading…</div>) : error ? (<div className="p-2 border border-red-300 bg-red-50 text-red-700 rounded">{error}</div>) : committee && (
        <>
          <div className="card">
            <div className="text-xl font-semibold">{committee.name}</div>
            {committee.responsibility && (<div className="text-sm text-gray-700 mt-1">{committee.responsibility}</div>)}
            <div className="mt-2 text-sm">Chair: {committee.chair ? `${committee.chair.firstName} ${committee.chair.lastName}` : '—'}</div>
          </div>
          ...
          <div className="card">
            ...
            <CommitteeMinutes name={committee.name} />
          </div>
        </>
      )}
    </div>
...
function CommitteeMinutes({ name }: { name: string }) {
